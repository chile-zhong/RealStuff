...
public class SearchViewModel extends ViewModel {
    ...
    private final LiveData<Result<List<SearchBean>>> mResult;

    public SearchViewModel() {
        mReposity = SearchReposity.getInstance();

        mSearchWrapper = new MutableLiveData<>();
        mResult = Transformations.switchMap(mSearchWrapper, new Function<SearchWrapper, LiveData<Result<List<SearchBean>>>>() {
            @Override
            public LiveData<Result<List<SearchBean>>> apply(SearchWrapper wrapper) {
    ...
    public LiveData<Result<List<SearchBean>>> getSearchResult() {
        return mResult;
    }
